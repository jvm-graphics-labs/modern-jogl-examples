 ...
 * @author gbarbieri
 */
public class cpuPositionOffset implements GLEventListener {
    ...
    public static void main(String[] args) {
        cpuPositionOffset positionOffset = new cpuPositionOffset();

        Frame frame = new Frame("Tutorial 03 - Position Offset");
...
//        System.out.println("display");
    ...
    private void AdjustVertexData(GL3 gl3) {
        float[] newVertexPosition = new float[12];
        System.arraycopy(vertexPositions, 0, newVertexPosition, 0, vertexPositions.length);
        
        for (int iVertex = 0; iVertex < vertexPositions.length; iVertex += 4) {
            newVertexPosition[iVertex] += fXoffset;
            newVertexPosition[iVertex + 1] += fYoffset;
        }

        gl3.glBindBuffer(GL3.GL_ARRAY_BUFFER, positionBufferObject[0]);
        {
            FloatBuffer buffer = GLBuffers.newDirectFloatBuffer(newVertexPosition);
            gl3.glBufferSubData(GL3.GL_ARRAY_BUFFER, 0, newVertexPosition.length * 4, buffer);
        }
    ...
    private void buildShaders(GL3 gl3) {
        ...
        programObject.attachVertexShader(gl3, shadersFilepath + "standard_VS.glsl");
