...
public class BasicImpostor extends Framework {
    ...
    private static class MaterialBlock {
        ...
        private ByteBuffer buffer = ByteBuffer.allocate(SIZE).order(ByteOrder.nativeOrder());
    ...
    public void display(GL3 gl) {
        ...
        LightBlock.lights[0].cameraSpaceLightPos.put(worldToCamMat.times(new Vec4(0.707f, 0.707f, 0.0f, 0.0f)));
        LightBlock.lights[0].lightIntensity.put(0.6f, 0.6f, 0.6f, 1.0f);

        LightBlock.lights[1].cameraSpaceLightPos.put(worldToCamMat.times(calcLightPosition()));
        LightBlock.lights[1].lightIntensity.put(0.4f, 0.4f, 0.4f, 1.0f);

        gl.glBindBuffer(GL_UNIFORM_BUFFER, bufferName.get(Buffer.LIGHT));
        gl.glBufferSubData(GL_UNIFORM_BUFFER, 0, LightBlock.SIZE, LightBlock.toBuffer());
        gl.glBindBuffer(GL_UNIFORM_BUFFER, 0);
    ...
    private void drawSphereOrbit(GL3 gl, MatrixStack modelMatrix, Vec3 orbitCenter, Vec3 orbitAxis, float orbitRadius,
        ...
        if (offsetDir.length() < 0.001f)
            orbitAxis.cross_(new Vec3(1.0f, 0.0f, 0.0f));
    ...
    private static class LightBlock {
        ...
        public static Vec4 ambientIntensity = new Vec4();
        public static float lightAttenuation;
        public static float[] padding = new float[3];
        public static PerLight[] lights = {new PerLight(), new PerLight()};

        public static ByteBuffer buffer = GLBuffers.newDirectByteBuffer(SIZE);

        public static ByteBuffer toBuffer() {
