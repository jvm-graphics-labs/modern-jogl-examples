...
fun main(args: Array<String>) {
    GeomImpostor_Next().setup("Tutorial 13 - Geometry Impostor")
}

class GeomImpostor_Next : Framework() {
    ...
    fun createMaterials(gl: GL3) = with(gl) {
        ...
        withUniformBuffer(bufferName[Buffer.MATERIAL_ARRAY]) { data(arrayBuffer, GL_STATIC_DRAW) }
        ...
        withUniformBuffer(bufferName[Buffer.MATERIAL_TERRAIN]) { data(terrainBuffer, GL_STATIC_DRAW) }
    ...
    override fun display(gl: GL3) = with(gl) {
        ...
        run {
            ...
            usingProgram(litMeshProg.theProgram) {
                litMeshProg.modelToCameraMatrixUnif.mat4 = modelMatrix.top()
                litMeshProg.normalModelToCameraMatrixUnif.mat3 = normMatrix
        ...
        run {
            ...
            withUniformBuffer(bufferName[Buffer.IMPOSTER]) { data(impostorBuffer, GL_STREAM_DRAW) }
        }
        ...
        run {
            ...
            usingProgram(litImpProg.theProgram) {
                withVertexArray(imposterVAO) { glDrawArrays(GL_POINTS, NUMBER_OF_SPHERES) }
            }
