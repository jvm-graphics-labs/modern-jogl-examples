...
import uno.buffer.destroyBuffer
...
class BlinnVsPhongLighting_() : Framework("Tutorial 11 - Blinn vs Phong Lighting") {
    ...
    override fun end(gl: GL3) = with(gl) {
        ...
        for (programPair in programs) {
            glDeleteProgram(programPair.whiteProgram.theProgram)
            glDeleteProgram(programPair.colorProgram.theProgram)
        }
    ...
    class ProgramData(gl: GL3, vertex: String, fragment: String) {
        ...
        var theProgram = programOf(gl, this::class.java, "tut11", vertex, fragment)

        var modelToCameraMatrixUnif = gl.glGetUniformLocation(theProgram, "modelToCameraMatrix")

        var lightIntensityUnif = gl.glGetUniformLocation(theProgram, "lightIntensity")
        var ambientIntensityUnif = gl.glGetUniformLocation(theProgram, "ambientIntensity")

        var normalModelToCameraMatrixUnif = gl.glGetUniformLocation(theProgram, "normalModelToCameraMatrix")
        var cameraSpaceLightPosUnif = gl.glGetUniformLocation(theProgram, "cameraSpaceLightPos")

        var lightAttenuationUnif = gl.glGetUniformLocation(theProgram, "lightAttenuation")
        var shininessFactorUnif = gl.glGetUniformLocation(theProgram, "shininessFactor")
        var baseDiffuseColorUnif = gl.glGetUniformLocation(theProgram, "baseDiffuseColor")
    ...
    object MaterialParameters {
        ...
        internal fun getSpecularValue(model: Int): Float {

            when (model) {

                BlinnVsPhongLighting.LightingModel.PhongSpecular, LightingModel.PhongOnly -> return phongExponent

                else -> return blinnExponent
            }
        }

        internal fun increment(model: Int, isLarge: Boolean) {

            when (model) {

                BlinnVsPhongLighting.LightingModel.PhongSpecular, LightingModel.PhongOnly -> phongExponent += if (isLarge) 0.5f else 0.1f
        ...
        internal fun decrement(model: Int, isLarge: Boolean) {
        ...
        fun clampParam(model: Int) {
            ...
            when (model) {
                ...
                BlinnVsPhongLighting.LightingModel.PhongSpecular, LightingModel.PhongOnly -> if (phongExponent <= 0.0f)
                    phongExponent = 0.0001f

                else -> if (blinnExponent <= 0.0f)
